import { useEffect, useRef, useState, useMemo } from 'react'
import * as d3 from 'd3'
import type { Topic, FlashEdge } from '../types'

interface GraphViewProps {
  topics: Topic[]
  accent: string
  onSelectTopic: (id: string) => void
  selectedId: string | null
  density: number
  showMinimap?: boolean
  recentIds: string[]
  flashEdgeIds: FlashEdge[]
  compact?: boolean
}

interface D3Node extends Topic {
  x?: number
  y?: number
  vx?: number
  vy?: number
  fx?: number | null
  fy?: number | null
}

interface D3Link {
  source: string | D3Node
  target: string | D3Node
}

function statusColor(status: Topic['status'], accent: string): string {
  if (status === 'mastered') return '#22c55e'
  if (status === 'in_progress') return '#f59e0b'
  if (status === 'available') return accent
  return '#2a2a2a'
}

function renderMinimap(svgEl: SVGSVGElement, nodes: D3Node[]) {
  const sel = d3.select(svgEl)
  const w = svgEl.clientWidth || 140
  const h = svgEl.clientHeight || 100
  const xs = nodes.map((n) => n.x ?? 0)
  const ys = nodes.map((n) => n.y ?? 0)
  if (xs.length === 0) return
  const minX = Math.min(...xs)
  const maxX = Math.max(...xs)
  const minY = Math.min(...ys)
  const maxY = Math.max(...ys)
  const sx = w / (maxX - minX + 40)
  const sy = h / (maxY - minY + 40)
  const s = Math.min(sx, sy)
  sel.selectAll('*').remove()
  const g = sel
    .append('g')
    .attr(
      'transform',
      `translate(${(w - (maxX - minX) * s) / 2 - minX * s}, ${(h - (maxY - minY) * s) / 2 - minY * s})`,
    )
  g.selectAll('circle')
    .data(nodes)
    .enter()
    .append('circle')
    .attr('cx', (d) => d.x ?? 0)
    .attr('cy', (d) => d.y ?? 0)
    .attr('r', (d) => Math.max(1, d.complexity * 0.3))
    .attr('fill', (d) => {
      switch (d.status) {
        case 'mastered':
          return '#22c55e'
        case 'in_progress':
          return '#f59e0b'
        case 'available':
          return '#3b82f6'
        default:
          return '#3a3a3a'
      }
    })
    .attr('opacity', (d) => (d.status === 'locked' ? 0.4 : 0.9))
}

export default function GraphView({
  topics,
  accent,
  onSelectTopic,
  selectedId,
  density,
  showMinimap = false,
  recentIds,
  flashEdgeIds,
  compact = false,
}: GraphViewProps) {
  const svgRef = useRef<SVGSVGElement>(null)
  const wrapRef = useRef<HTMLDivElement>(null)
  const minimapRef = useRef<SVGSVGElement>(null)
  const [tooltip, setTooltip] = useState<{
    x: number
    y: number
    name: string
    status: Topic['status']
    complexity: number
  } | null>(null)

  const links = useMemo<D3Link[]>(() => {
    const all: D3Link[] = []
    const ids = new Set(topics.map((t) => t.id))
    topics.forEach((t) => {
      ;(t.prereqs ?? []).forEach((p) => {
        if (ids.has(p)) all.push({ source: p, target: t.id })
      })
    })
    return all
  }, [topics])

  useEffect(() => {
    const wrap = wrapRef.current
    const svgEl = svgRef.current
    if (!wrap || !svgEl) return
    const W = wrap.clientWidth
    const H = wrap.clientHeight

    const nodes: D3Node[] = topics.map((t) => ({ ...t }))
    const linkData: D3Link[] = links.map((l) => ({ source: l.source, target: l.target }))

    const sim = d3
      .forceSimulation<D3Node>(nodes)
      .force(
        'link',
        d3
          .forceLink<D3Node, D3Link>(linkData)
          .id((d) => d.id)
          .distance((d) => {
            const src = d.source as D3Node
            const tgt = d.target as D3Node
            return 60 + Math.abs((src.complexity || 1) - (tgt.complexity || 1)) * 8 * density
          })
          .strength(0.4),
      )
      .force('charge', d3.forceManyBody<D3Node>().strength(-260 * density))
      .force('center', d3.forceCenter(W / 2, H / 2))
      .force('collide', d3.forceCollide<D3Node>().radius((d) => 8 + d.complexity * 1.6 + 8))
      .alphaDecay(0.025)

    const svg = d3.select(svgEl)
    svg.selectAll('*').remove()

    // Defs
    const defs = svg.append('defs')
    defs
      .append('marker')
      .attr('id', 'arrow')
      .attr('viewBox', '0 -5 10 10')
      .attr('refX', 16)
      .attr('refY', 0)
      .attr('markerWidth', 5)
      .attr('markerHeight', 5)
      .attr('orient', 'auto')
      .append('path')
      .attr('d', 'M0,-5L10,0L0,5')
      .attr('fill', '#3a3a3a')

    const glow = defs
      .append('filter')
      .attr('id', 'node-glow')
      .attr('x', '-100%')
      .attr('y', '-100%')
      .attr('width', '300%')
      .attr('height', '300%')
    glow.append('feGaussianBlur').attr('stdDeviation', '4').attr('result', 'blur')
    glow
      .append('feMerge')
      .selectAll('feMergeNode')
      .data(['blur', 'SourceGraphic'])
      .enter()
      .append('feMergeNode')
      .attr('in', (d: string) => d)

    const root = svg.append('g').attr('class', 'zoom-root')

    const linkSel = root
      .append('g')
      .attr('class', 'links')
      .selectAll('path')
      .data(linkData)
      .enter()
      .append('path')
      .attr('class', 'edge')
      .attr('opacity', (d) => {
        const s = (d.source as D3Node).status
        const t = (d.target as D3Node).status
        if (s === 'locked' || t === 'locked') return 0.18
        return 0.45
      })
      .attr('stroke', (d) => {
        const t = (d.target as D3Node).status
        if (t === 'mastered') return '#2a4a36'
        return '#2a2a2a'
      })
      .attr('marker-end', 'url(#arrow)')

    const nodeSel = root
      .append('g')
      .attr('class', 'nodes')
      .selectAll('g')
      .data(nodes)
      .enter()
      .append('g')
      .attr('class', (d) => `node node-${d.status}`)
      .style('cursor', (d) => (d.status === 'locked' ? 'not-allowed' : 'pointer'))
      .on('mouseenter', (event: MouseEvent, d: D3Node) => {
        setTooltip({ x: event.clientX, y: event.clientY, name: d.name, status: d.status, complexity: d.complexity })
        d3.select(event.currentTarget as Element)
          .select('.node-circle')
          .attr('r', 8 + d.complexity * 1.6 + 2)
      })
      .on('mousemove', (event: MouseEvent) => {
        setTooltip((t) => (t ? { ...t, x: event.clientX, y: event.clientY } : null))
      })
      .on('mouseleave', (event: MouseEvent, d: D3Node) => {
        setTooltip(null)
        d3.select(event.currentTarget as Element)
          .select('.node-circle')
          .attr('r', 8 + d.complexity * 1.6)
      })
      .on('click', (_event: MouseEvent, d: D3Node) => {
        if (d.status === 'locked') return
        onSelectTopic(d.id)
      })

    // Pulse halo for available nodes
    nodeSel
      .filter((d) => d.status === 'available')
      .append('circle')
      .attr('class', 'pulse-halo')
      .attr('r', 0)
      .attr('fill', 'none')
      .attr('stroke', accent)
      .attr('stroke-width', 1.5)
      .attr('opacity', 0)
      .style('animation', 'node-pulse 2.4s ease-in-out infinite')

    nodeSel
      .append('circle')
      .attr('class', 'node-circle')
      .attr('r', (d) => 8 + d.complexity * 1.6)
      .attr('fill', (d) => {
        switch (d.status) {
          case 'locked':
            return '#0a0a0a'
          case 'available':
            return accent
          case 'in_progress':
            return '#f59e0b'
          case 'mastered':
            return '#22c55e'
          default:
            return '#3a3a3a'
        }
      })
      .attr('stroke', (d) => statusColor(d.status, accent))
      .attr('stroke-width', (d) => (d.status === 'locked' ? 1 : 1.5))
      .attr('filter', (d) => (d.status === 'available' ? 'url(#node-glow)' : null))
      .attr('opacity', (d) => (d.status === 'locked' ? 0.55 : 1))

    nodeSel
      .append('text')
      .attr('class', 'node-label')
      .attr('dy', (d) => 8 + d.complexity * 1.6 + 14)
      .text((d) => d.name)

    // Selected ring
    nodeSel
      .filter((d) => d.id === selectedId)
      .append('circle')
      .attr('class', 'selected-ring')
      .attr('r', (d) => 8 + d.complexity * 1.6 + 6)
      .attr('fill', 'none')
      .attr('stroke', '#f1f1f1')
      .attr('stroke-width', 1)
      .attr('stroke-dasharray', '2 3')

    // Recent ring indicator
    nodeSel
      .filter((d) => recentIds.includes(d.id) && d.id !== selectedId)
      .append('circle')
      .attr('r', 2)
      .attr('cx', (d) => 8 + d.complexity * 1.6)
      .attr('cy', (d) => -(8 + d.complexity * 1.6))
      .attr('fill', '#f1f1f1')

    // Zoom
    const zoomBehavior = d3
      .zoom<SVGSVGElement, unknown>()
      .scaleExtent([0.3, 3])
      .on('zoom', (event) => {
        root.attr('transform', event.transform)
      })
    svg.call(zoomBehavior)

    sim.on('tick', () => {
      linkSel.attr('d', (d) => {
        const src = d.source as D3Node
        const tgt = d.target as D3Node
        const dx = (tgt.x ?? 0) - (src.x ?? 0)
        const dy = (tgt.y ?? 0) - (src.y ?? 0)
        const dr = Math.sqrt(dx * dx + dy * dy) * 1.6
        return `M${src.x ?? 0},${src.y ?? 0}A${dr},${dr} 0 0,1 ${tgt.x ?? 0},${tgt.y ?? 0}`
      })
      nodeSel.attr('transform', (d) => `translate(${d.x ?? 0},${d.y ?? 0})`)
      if (minimapRef.current) {
        renderMinimap(minimapRef.current, nodes)
      }
    })

    const ro = new ResizeObserver(() => {
      const nw = wrap.clientWidth
      const nh = wrap.clientHeight
      if (nw > 0 && nh > 0) {
        sim.force('center', d3.forceCenter(nw / 2, nh / 2))
        sim.alpha(0.5).restart()
      }
    })
    ro.observe(wrap)

    return () => {
      ro.disconnect()
      sim.stop()
    }
  }, [topics, links, density]) // eslint-disable-line react-hooks/exhaustive-deps

  // Update accent without rebuilding sim
  useEffect(() => {
    if (!svgRef.current) return
    const svg = d3.select(svgRef.current)
    svg.selectAll<SVGCircleElement, D3Node>('.node-available .node-circle').attr('fill', accent).attr('stroke', accent)
    svg.selectAll('.pulse-halo').attr('stroke', accent)
  }, [accent])

  // Update selection ring
  useEffect(() => {
    if (!svgRef.current) return
    const svg = d3.select(svgRef.current)
    svg.selectAll('.selected-ring').remove()
    if (!selectedId) return
    svg
      .selectAll<SVGGElement, D3Node>('.node')
      .filter((d) => d.id === selectedId)
      .append('circle')
      .attr('class', 'selected-ring')
      .attr('r', (d) => 8 + d.complexity * 1.6 + 6)
      .attr('fill', 'none')
      .attr('stroke', '#f1f1f1')
      .attr('stroke-width', 1)
      .attr('stroke-dasharray', '2 3')
  }, [selectedId])

  // Flash edges on unlock
  useEffect(() => {
    if (!flashEdgeIds.length || !svgRef.current) return
    const svg = d3.select(svgRef.current)
    svg.selectAll<SVGPathElement, D3Link>('.edge').each(function (d) {
      const sId = typeof d.source === 'object' ? (d.source as D3Node).id : d.source
      const tId = typeof d.target === 'object' ? (d.target as D3Node).id : d.target
      if (flashEdgeIds.some((e) => e.from === sId && e.to === tId)) {
        d3.select(this).classed('unlocked-flash', true)
        setTimeout(() => d3.select(this).classed('unlocked-flash', false), 1400)
      }
    })
  }, [flashEdgeIds])

  return (
    <div ref={wrapRef} className="graph-canvas-wrap">
      <svg ref={svgRef} />
      {!compact && (
        <div className="legend">
          <div className="legend-row">
            <span className="legend-dot" style={{ background: '#22c55e' }} />
            <span>Mastered</span>
          </div>
          <div className="legend-row">
            <span className="legend-dot" style={{ background: '#f59e0b' }} />
            <span>In progress</span>
          </div>
          <div className="legend-row">
            <span className="legend-dot" style={{ background: accent }} />
            <span>Available</span>
          </div>
          <div className="legend-row">
            <span className="legend-dot" style={{ background: '#2a2a2a', border: '1px solid #3a3a3a' }} />
            <span>Locked</span>
          </div>
        </div>
      )}
      {!compact && (
        <div className="graph-overlay-stats">
          <div className="stat-card">
            <div className="stat-num">
              {topics.filter((t) => t.status === 'mastered').length}
              <span style={{ color: 'var(--text-mute)', fontSize: 13 }}>/{topics.length}</span>
            </div>
            <div className="stat-label">Mastered</div>
          </div>
          <div className="stat-card">
            <div className="stat-num">{topics.filter((t) => t.status === 'in_progress').length}</div>
            <div className="stat-label">In progress</div>
          </div>
          <div className="stat-card">
            <div className="stat-num">{topics.filter((t) => t.status === 'available').length}</div>
            <div className="stat-label">Available</div>
          </div>
        </div>
      )}
      {!compact && recentIds.length > 0 && (
        <div className="recent-panel">
          <div className="recent-header">
            <span>Recently studied</span>
            <span>{recentIds.length}</span>
          </div>
          <div className="recent-list">
            {recentIds.map((id) => {
              const t = topics.find((x) => x.id === id)
              if (!t) return null
              return (
                <div key={id} className="recent-item" onClick={() => onSelectTopic(id)}>
                  <span className="recent-status" style={{ background: statusColor(t.status, accent) }} />
                  <span className="recent-name">{t.name}</span>
                </div>
              )
            })}
          </div>
        </div>
      )}
      {showMinimap && (
        <div className="minimap">
          <span className="minimap-label">Map</span>
          <svg ref={minimapRef} />
        </div>
      )}
      {tooltip && (
        <div className="tooltip" style={{ left: tooltip.x + 14, top: tooltip.y + 14 }}>
          <div>{tooltip.name}</div>
          <div className="tt-status">
            {tooltip.status === 'locked'
              ? 'Locked — complete prerequisites first'
              : `${tooltip.status.replace('_', ' ')} · complexity ${tooltip.complexity}/10`}
          </div>
        </div>
      )}
    </div>
  )
}
